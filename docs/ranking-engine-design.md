...
| 8 | 同一份 SKILL.md 摆进多个宿主目录 | **按全文哈希合成一张卡**，见 §16.2。键取全文而不是正文——`larksuite/cli` 的 `lark-minutes` / `lark-note` 正文逐字节相同、只有 frontmatter 不同，是两个真 skill |
| 9 | `completeness_score` 的长度尺子 | **按信息量而不是字符数**，CJK 一个字顶两个拉丁字符，见 §16.3。旧口径对中英文是两把尺子 |
| 10 | 「装到本机」按钮放不放公开站 | **不放**。整段代码按变体在生成时决定，公开 `index.html` 里连一个相关字节都没有，见 §17 |
```
...
## 16. 供给端成色：星数不是 skill 的分，completeness 得真有区分度

### 16.1 先测：`global_score` 实际是个常数

线上 554 条实测，`global_score` 的 p10→p90 只有 **0.067**，取值只有 45 种。逐项拆开
看贡献（权重 × 该项自己的 p10→p90 跨度），能看出钱花在哪：

| 分项 | 权重 | 该项跨度 | 对最终跨度的贡献 |
| --- | --- | --- | --- |
| `ctr` | 0.34 | 0.000 | **0.0000** |
| `vel` | 0.28 | 0.160 | 0.0448 |
| `stock` | 0.14 | 0.100 | 0.0140 |
| `fresh` | 0.12 | 0.050 | 0.0060 |
| `completeness` | 0.12 | 0.000 | **0.0000** |

**权重最大的那一路贡献恰好是 0**。`ctr` 恒等于先验 0.50，因为静态站没有曝光回传，
`ItemStats` 全空——这不是 bug，是 §12 那条链路还没落地的直接后果，归到 `p1-measure`
下游，回传通了自然有值。在那之前，把 0.34 的权重挂在一个常数上，等于把真正在区分
条目的三路（vel / stock / completeness）联合压缩到 0.66 的量程里。

`completeness` 的 0 则是能修的，下面两节是它。

顺带一个测准的结论：**星数是真的，但它是仓库粒度的**。554 张卡只有 49 种取值，
92.4% 是 monorepo 子条目（同一个母仓中位摊 20 张卡），所以一个仓里 20 个 skill
共享同一个星数。抽样打 GitHub 页面核对过，数字本身没错——它只是回答不了"这条
skill 好不好"。稀释已由 `annotate_repo_share` 处理，不重复造轮子。

### 16.2 去重：一条能力摊成多张卡

16 张重复卡，两个互不相干的成因：

**soft 池不自去重（4 张）**。`feed_pack.soft_skills_from_corpus` 的入参池子是
`skill_pool + corpus_rows` 拼的，两段读同一份 corpus，同一条 skill 出现两次。函数
只挡了调用方传进来的 `exclude`，没挡自己刚吐出去的键，于是线上出现 id 一模一样的两张卡。

**同一份 SKILL.md 摆进多个宿主目录（12 张）**。一个仓为了同时伺候 Claude / Codex /
Cursor，常把同一份文件原样放进 `.claude/skills/x/`、`.agents/skills/x/`、
`.cursor/skills/x/`。id 是 `full_name::skill_path`，路径不同 id 就不同，按 id 去重
认不出来——`TanStack/ai` 一家就多出 9 张。改成按 `SKILL.md` 内容的 SHA256 去重。

**去重键必须是全文，不能是正文**：`larksuite/cli` 的 `lark-minutes` 与 `lark-note`
正文逐字节相同（都是几十字的兼容壳），只有 frontmatter 里的名字和描述不同，是两个
独立的真 skill。拿正文当键会合掉一个。这条已实网验证，也写成了测试。

去重省下的额度要**拿去装真 skill**，而不是白丢：`enrich_repo_multi` 先按哈希过滤
再计数，凑够 `max_skills` 个**不同内容**才停。`skill_paths`（"在哪儿找到过"的记录）
保留全部原始路径，不被去重结果削薄。

### 16.3 completeness 重做：读对字段、量对长度、看结构

旧版四项里三项是常数（`skill_path` 命中 98.6%、`highlights` 100%、`body_preview`
98.6%），554 条只落在 3 个取值上、96% 挤在 0.75。三个独立的毛病：

**读错字段**。旧口径 `one_liner_zh or description`，而 `one_liner_zh` 是 i18n 阶段
才生成的、比打分晚——于是同一个函数在流水线时点和事后重算时点给出两个答案（实测
13.2% 不一致）。更糟的是 `one_liner_zh` 中位 28 字，把中位 244 字的 `description`
整个挡在门外。现在只读**打分时就存在、且会随 `feed.json` 发布**的字段。

**量错长度**。`len(desc) >= 40` 对中英文是两把尺子：`one_liner_en` 中位 83 字符、
99.8% 过线；`one_liner_zh` 中位 28 字、只有 9.2% 过线。同一个 skill 翻成中文就
"不合格"，翻成英文就"合格"，这是量纲错误不是质量差异。`text_weight()` 里 CJK
一个字按两个拉丁字符算。阈值也从悬崖改成斜坡（`_ramp`），阈值选偏一点不会翻盘。

**没看真正能区分的东西**。正文结构才有分布：小节 81%、列表 57%、代码块 22%。
新口径 = 描述充实度 0.38 + 正文结构 0.50 + 出处 0.12，权重与阈值全在
`config_defaults.json` 的 `completeness` 段里，可运行时调。

**时点稳定**是硬约束，测试锁死两条：斜坡终点必须落在 `normalize_item` 的截断线
之内（描述 400 / 正文 700），且只读会发布的字段。否则"什么时候算"又会变成"算出多少"。

结果：`completeness` 从 3 个取值散成 p10 0.53 → p90 0.90，对 `global_score` 跨度的
贡献从 0.0000 变成 **0.0442**；`global_score` 总跨度 0.067 → **0.091**，取值 45 → 252 种，
中位名次移动 62 位、11 条纹丝不动。兼容壳（`lark-minutes` 这类）如预期沉到底部。

39 条测试 + 20 种变异全部抛住，覆盖：读哪个字段、CJK 双倍权重、时点不漂移、
结构三项各自加分、权重是配置不是硬编码、两条去重路径各自的边界。

## 17. 「装到本机」按钮：只长在本机看板上

引擎在 skill-picker 那边（`installer.py` + `install_api.py`），feed 这边只出按钮。
按钮**不进公开站**：`INSTALL_CSS` / `INSTALL_SHEET_HTML` / `INSTALL_JS` 三块按变体在
**生成时**决定要不要写进去，公开 `index.html` 里连 `js-install`、`/api/install`、
「装到本机」这些字节都搜不到，不靠运行时 if 判断。测试逐字节盯着这件事。

运行档由地址栏决定，不由配置决定：`127.0.0.1` / `localhost` → `live`（真能装）；
`file://` → `copy`（给一条可复制的命令）；其它 → `off`（按钮根本不渲染）。

### 17.1 两个只有真浏览器抓得住的 bug

Node 测试 88 条全绿、实网 API 也通，但页面一片空白——两个 bug 都不在被测代码里，
而在**代码之间**：

**插入位置（暂时性死区）**。`cardHtml` 会调 `installBtnHtml`，而它读 `PICKER`。
函数声明会提升，`const PICKER` 不会。这段原来插在脚本末尾、`applyLang(LANG)`
那次首屏渲染之后——于是每张卡都在 TDZ 上抛 `ReferenceError`，整个 feed 渲成空白。
node harness 抓不到，因为作用域是 harness 自己拼的、顺序是它定的。现在插在首屏
渲染之前，并用**源码顺序断言**锁住（`const PICKER` 必须早于 `applyLang(LANG)`）。

**参数语义对不上**。后端 `path` 要的是「装哪个子目录」，feed 的 `skill_path` 指到
`SKILL.md` 文件本身。直接递过去，后端去找 `skills/x/SKILL.md/SKILL.md`，实网报
「在包里不存在」。加了 `installSubdir()` 削掉末尾的 `*.md` 段，8 条边界进了测试。

教训写这儿：**跨进程的参数语义、跨语句的求值顺序，单元测试都照不到**。这类接缝
只能靠一次真实点击。

### 17.2 实网走完的一圈

`plan`（37 文件 / 237.2 KB / commit `853622144588` / 落地路径全列出，一个字节没写盘）
→ `确认安装` → 落盘校验 → 重扫 → 进 catalog → 五道门禁 → 卡片徽标当场翻成
「本机已有同名」（`LOCAL_FRESH` 覆盖层，不等下一轮扫描）→ 回执「写了 37 个文件」。
再 `remove --yes` 回滚，目录与 `installed.json` 都干净。

同名那条也验了：`stop-slop` 本机已有，`plan` 出的是**红字阻塞**加「换个名字 / 先
remove」的出路，`确认安装` 按钮根本不渲染——只剩「不装」。
