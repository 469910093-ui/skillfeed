...
用户可浏览官方发现流（**全量免费，不按天切条**），登录后发布自己的 skill（UGC）。变现以**技能包低价获客**为主入口，增值走人工服务与付费资讯（见 §3.1）；不拿浏览设墙。
...
**产品货架（full 底栏「一站式配齐」）：** 发现流仍全量免费。**场景包无买断**——卖的是**月 / 季 / 年订阅**，期内按席位选用场景并持续更新；过期后手册与席位访问停用。首波可订阅：货架电商、跨境电商、短剧、营销广告、数据分析；内容电商 / 直播电商 / 资讯 / 平面设计标「即将推出」。**商详**仍用环节大图 + 成套亮点，不展开手册正文。

| 轨道 | 月 | 季 | 年 | 席位 |
|---|---|---|---|---|
| **连续续费**（默认推，更便宜） | ¥29 | ¥78 | ¥228 | 1 / 2 / 3 |
| **单独买同周期**（不自动续） | ¥39 | ¥105 | ¥299 | 1 / 2 / 3 |

SKU：`subscriber_month_cont` / `quarter_cont` / `year_cont` · `subscriber_month_once` / `quarter_once` / `year_once`（旧 `subscriber_year` 等同连续包年）。收银台 `GET /api/pay/catalog`；下单 `POST /api/pay/checkout`（可带 `assign_pack` 顺带占席）；已订阅占席 `POST /api/me/seats/assign` 或整表 `PUT /api/me/seats`。交付 `GET /api/packs/{id}/delivery` 仅**订阅有效且席位占用**时下发四样：手册、技能排序、应用介绍、知识库。占用超过席位上限时，开通或读取会收掉多余项：保留最近占用，本次开通指定的场景优先留下。商详入口价：**连续包月 ¥29 起**；CTA 默认开连续包月并占用当前场景。即将推出不能下单。`SKILLFEED_DEV=1` 时 checkout 直接到账；生产未接微信支付时拒绝开通。目录真相源：飞书 Base `PCWZbMvydabY2Xs1yYSc2sc7nth` → `docs/packs/shelf_catalog.json`（`python scripts/sync_packs_shelf.py`）。深链：`?tab=packs` / `?tab=packs&pack=cross-border`。lite 不露出该 tab。

**增值层（利润，不替代订阅）：** ① **人工服务**——装机答疑 / 岗位陪跑 / 定制配齐（站内入口见下「找策展人」）；② **付费资讯**——飞得周讯。装机引导：订阅开通后提示「装到本机后，推荐会按你的上下文更新」。

**找策展人（用户如何找到「我」）：** 主通道定为 **Skill Man 公众号后台**（资讯推送、人工预约、订阅答疑都在号内办；站内不另建客服系统优先）。公开站不在发现流挂私人微信号。站内触点只做「导到号」：技能包商详底部与开通成功页「去 Skill Man 问策展人」；「我的 → 场景席位」旁同入口（文案可带包名，方便用户复制到号内）；页脚 / `about`「谁在策展」出公众号名 + 二维码；你的发布者主页「合作 / 咨询」同链。号内菜单建议：`订阅答疑` / `预约人工` / `本周资讯` / `回技能货架`（链回 `?tab=packs`）。小红书等站外仍统一署名，最终收回 Skill Man。页上写清：工作日多久内公众号回复。
...
| **一站式配齐** | 货架顶部是场景订阅入口：两句亮点（按本机已有技能推荐、所选场景持续更新）+ 连续月/季/年与单独买同周期。下方再选场景占席。未登录去登录再 `POST /api/pay/checkout`；lite 隐藏 | P0 |
...
|---|---|---|---|---|---|---|
| **同源有后端** | `API_BASE` 且同源（含主站漏写 `api_base` 时回退 `https://skillfeeder.cn`） | ✅ | ✅ | 货架 + 登录后结账开通 | 页内表单 → `/api/posts` | 真实登录态 + 我发布的 |
...
地址栏已有非发现 tab（含购买回跳 `?tab=packs`）时不启动，避免引导把人拽回发现并改掉深链。
...
| 2026-09-23 | 购买后同一页交付手册、技能排序、应用介绍、知识库；未购买不下发正文 | §3.1、§5.1、`server/pack_delivery.py`、`feed_dashboard.py` |
| 2026-09-23 | 货架导语去掉「骨架/待补采/飞书表」等对内说明，只留用户能读的一句 | §5.1、`shelf_catalog.json` |
| 2026-09-23 | 登录回跳的 `next` 按 JS 字符串注入，购买后能回到 `?tab=packs&pack=`；不再把 `&` 写成 `&amp;` | `login.html`、`server/app.py` |
| 2026-09-23 | 首访引导遇到 `?tab=` 深链（购买回跳、直接开技能包）时不启动，避免把人拽回发现 | §5.1、`feed_dashboard.py` |
| 2026-09-23 | 可买技能包标价 ¥39；购买走登录 → `/api/pay/checkout` → `pack_grants` 解锁手册。开发模式直接到账；未接微信支付时拒绝开通 | §3.1、§5.1、`server/pay.py`、`feed_dashboard.py`、`shelf_catalog.json` |
| 2026-09-22 | 「找策展人」主通道定为 Skill Man 公众号：站内只导号（商详/购后/已购/about），号内办资讯·预约·答疑；不优先自建客服 | §3.1 |
| 2026-09-22 | 变现定调：技能包为最小可售单元、低价获客（¥29–49）；增值走人工服务与付费资讯；站内「找策展人」触点；本机 Pro/大类年卡暂不主推 | §3.1 |
