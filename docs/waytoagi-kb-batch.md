...
> 原则：只把**能落到 GitHub 仓**的 Skills / Agent / 工具写入资源清单；纯资讯/白皮书/活动不进表。  
> 硬性规定：入库失败或 0 条成功 → **必须通知用户**，禁止假成功（见 `ops/kb/NOTIFY-ON-FAIL.md` / 本脚本 `INGEST_ALERT.md`）。
